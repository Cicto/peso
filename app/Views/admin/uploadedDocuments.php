...
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <h4 class = "card-title mb-4"><?= $clientInformation->firstname.' '.$clientInformation->lastname ?>'s Files</h4>
                <?php $count = 0;?>
                <?php foreach($documents as $docs):?>
                    <table class = "table table-bordered table-sm">
                        <thead class = "table-light">
                            <tr>
                                <th colspan = "4"><?= $docs['rt_description'] ?></th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php $rtId = $docs['rt_id'];?>
                            <?php foreach($docs['reqList'] as $req):?>
                                <tr>
                                    ...
                                    <td style = "width: 800px">
                                        <p class = "" style = "font-size: 15px"><?= $req['subDocDetails']['reqTitle']?>
                                        ...
                                        <br>
                                        <!-- <small class = "text-muted font-11"><?= mb_strimwidth($req['subDocDetails']['reqDesc'], 0, 100, "..."); ?></small> -->
                                    </td>
                                    <td>
                                        <!-- <button class = "btn btn-success btn-sm mt-1"><span class = "ri-add-fill"></span></button> -->
                                            ...
                                            <?php else:?>
                                                <?php foreach($req['uploadStatus'] as $req):?>
                                                    <div class="btn-group mt-1 ">
                                                        <a type="button" href = "<?= base_url()?>/public/uploads/documents/<?= $clientInformation->firstname.'_'.$clientInformation->lastname.'_'.$clientInformation->user_id.'/'.$req->filename?>" target = "_blank" class="btn btn-outline-success btn-xs" data-bs-toggle="tooltip" data-bs-placement="top" title data-bs-original-title="<?= $req->filename?>" ><span class = "ri-file-list-line"></span></a>
                                                        <a type="button" href = "<?= base_url()?>/public/uploads/documents/<?= $clientInformation->firstname.'_'.$clientInformation->lastname.'_'.$clientInformation->user_id.'/'.$req->filename?>" target = "_blank" class="btn btn-outline-success btn-xs" data-bs-toggle="tooltip" data-bs-placement="top" title data-bs-original-title="<?= $req->filename?>" download><span class = "ri-download-cloud-2-line"></span></a>
            ...
            <div class="card-body">
                <form enctype = "multipart/form-data" method = "POST" role = "form" id = "additional-form" data-appid = "<?= $applicationId ?>">
                    <div class="row">
                        ...
                        <div class="col-6">
                            <div class="mb-2">
                                <label for="requirement_file" class="form-label"></label>
                                <input type="file" class="form-control mt-1" id="requirement_file" name="requirement_file" required>
                            </div>
                        </div>
                    </div>
                        ...
                        <?php if($additionalDocs != FALSE):?>
                            <?php foreach($additionalDocs as $doc):?>
                                <tr>
                                    ...
                                    <td>
                                        <a class = "btn btn-primary btn-sm" href="<?= base_url().'/public/uploads/documents/'.$clientInformation->firstname.'_'.$clientInformation->lastname.'_'.$clientInformation->user_id.'/'.$doc->filename?>" target = "_blank"><span class = "ri-eye-line"></span></a>
                                        <a class = "btn btn-primary btn-sm" href="<?= base_url().'/public/uploads/documents/'.$clientInformation->firstname.'_'.$clientInformation->lastname.'_'.$clientInformation->user_id.'/'.$doc->filename?>" target = "_blank" download> <span class = "ri-download-cloud-2-line"></span>  </a>
                                    </td>
                                </tr>
                            <?php endforeach;?>
                        <?php endif;?>
...
<script src="<?= base_url()?>/public/assets/js/admin/uploadedDocuments.js"></script>
<?= $this->endSection(); ?>